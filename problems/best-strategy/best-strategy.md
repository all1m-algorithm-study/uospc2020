...
문기는 귀찮은 나머지 놓인 카드를 살펴보고 해당 차례에 뽑을 수 있는 2개의 카드에 적힌 수의 합이 최대가 되도록 뽑는다.
정현이는 문기가 그러한 전략을 사용했을 때, 정현이 본인이 뽑은 카드에 적힌 수의 합이 최대가 되도록 전략을 짜고 싶다.
이를 *정현이의 최선의 전략*이라고 한다.
정현이가 이러한 최선의 전략을 사용했을 때 정현이가 얻게 되는 카드의 합을 출력하여라.
...
## 출력
정현이가 최선의 전략을 사용했을 때 정현이가 얻게 되는 카드의 합을 출력하여라.
